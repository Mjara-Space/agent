...
SITE_METADATA_OPERATION_NAMES = frozenset({"language"})
...
def execute_site_metadata_operation(site: Any, operation: str) -> dict[str, str]:
	if operation not in SITE_METADATA_OPERATION_NAMES:
		raise ValueError("unsupported site metadata operation")

	result = site.bench_execute("console", input=_build_language_script())
